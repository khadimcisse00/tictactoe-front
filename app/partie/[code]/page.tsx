  ...
  useEffect(() => {
    async function initialiser() {
      ...
      socket.on("partie_info", (donnees) => {
        ...
        const grilleArray = typeof donnees.grille === "string"
        ...
        if (donnees.partieComplete) {
          setMessageEtat(`La partie peut commencer ! Vous êtes ${donnees.symbole}`);
        } else {
          setMessageEtat(`En attente d'un adversaire... Vous êtes ${donnees.symbole}`);
        }
      });
      ...
      <div className="text-center">
        ...
        {monSymbole && partieComplete && (
          <p className="text-sm mt-2">
            {cEstMonTour ? (
              <span className="text-success font-semibold">C'est votre tour !</span>
            ) : (
              <span className="text-base-content/70">En attente de l'adversaire...</span>
            )}
          </p>
        )}
      </div>
